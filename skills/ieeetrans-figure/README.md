...
`ieeetrans-figure` 衍生自 [`Yuan1z0825/nature-skills`](https://github.com/Yuan1z0825/nature-skills) 仓库中的 [nature-figure] 技能（commit `fac88c4`），该项目以 **Apache License 2.0** 授权。继承的 `static/`、`references/`、`scripts/`、`assets/` 目录为上游未修改内容；本技能自有内容为 `panels/`、`SKILL.md`、`manifest.yaml` 与本 README。本作品以相同的 Apache-2.0 许可发布（见仓库根目录 `LICENSE`）。
